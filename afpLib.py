...
def parse_FPLoginReply_DHX2(struc: bytes) -> (int,int, int, bytes, int):
    '''
    ...
    Mb = int.to_bytes(bytes(Mb),"big") # Serveer public key
...
def craft_FPLoginCont_DHX2(ID,g,pLength,p) -> bytes:
    global C2SIV, Ra
    '''
    ...
    '''
    Ra = randrange(0,pLength ** 2) # "private" key

    Ma = g^Ra % p # "public" key

    nOnce = randrange(0,16383) # (16B+8)² - 1 in case nOnce + 1 is over 16B

    cypher = CAST.new(mode=CAST.MODE_CBC,iv=C2SIV,key=Ra)
